...
def worker(
    ...
    def op_detail() -> None:
        client.get_run(random.choice(run_ids))

    def op_metrics() -> None:
        client.get_metrics(random.choice(run_ids), names=["accuracy"], max_points=200)

    def op_compare() -> None:
        chosen = random.sample(run_ids, min(4, len(run_ids)))
...
def main() -> None:
    ...
    with ThreadPoolExecutor(max_workers=concurrent_users) as pool:
        futures = [
            pool.submit(worker, args.server_url, args.api_key, project, run_ids, deadline)
    ...
    results = {
        ...
        "results": {
            "dashboard_load": summarize_ms(latency_values),
        },
    }
